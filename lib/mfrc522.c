...
void PCD_Init_IO(void)
{
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_0; // RST
...
void PCD_WriteRegister(	byte reg,		///< The register to write to. One of the PCD_Register enums.
		...
		SPI1_Write((reg & 0x7E));	// MSB == 0 is for writing. LSB is not used in address. Datasheet section 8.1.2.3.
		SPI1_Write(value);
...
byte PCD_ReadRegister(	byte reg	///< The register to read from. One of the PCD_Register enums.
	...
	byte value;
	MFRC522_CS_LOW;			// Select slave
	SPI1_Write(0x80 | (reg & 0x7E));			// MSB == 1 is for reading. LSB is not used in address. Datasheet section 8.1.2.3.
	value = SPI1_Write(0);					// Read the value back. Send 0 to stop reading.
	MFRC522_CS_HIGH;			// Release slave again
	return value;
...
void PCD_ReadRegisterMulti(	byte reg,		///< The register to read from. One of the PCD_Register enums.
	...
	SPI1_Write(address);					// Tell MFRC522 which address we want to read
	while (index < count) {
		if (index == 0 && rxAlign) { // Only update bit positions rxAlign..7 in values[0]
			...
			byte value = SPI1_Write(address);
			// Apply mask to both current value of values[0] and the new data in value.
			values[0] = (values[index] & ~mask) | (value & mask);
		}
		else { // Normal case
			values[index] = SPI1_Write(address);	// Read value and tell that we want to read the same address again.
		}
		index++;
	}
	values[index] = SPI1_Write(0);			// Read the final byte. Send 0 to stop reading.
	MFRC522_CS_HIGH;			// Release slave again
} // End PCD_ReadRegister()
